...
UI::draw(ge211::Sprite_set& set){
    ...
    /// stack too high
    if (model.get_set().find_height() < model.config.top_margin || model.get_set()
    ...
    if (!model.check_is_game_over() || model.get_set().find_max_tier() == model.config
    .fruit_tier){
        ...
        case 1: set.add_sprite(tier_1_sprite,model.get_generated_fruit().top_left
        ());
            break;
        case 2: set.add_sprite(tier_2_sprite,model.get_generated_fruit().top_left());
            break;
        case 3: set.add_sprite(tier_3_sprite,model.get_generated_fruit().top_left());
            break;
        case 4: set.add_sprite(tier_4_sprite,model.get_generated_fruit().top_left());
            break;
        case 5: set.add_sprite(tier_5_sprite,model.get_generated_fruit().top_left());
            break;
        case 6: set.add_sprite(tier_6_sprite,model.get_generated_fruit().top_left());
            break;
        case 7: set.add_sprite(tier_7_sprite,model.get_generated_fruit().top_left());
            break;
        }
    ...
    if (model.check_is_paused() && !model.check_is_game_over() && model.get_set()
    .find_max_tier()
...
UI::on_mouse_up(ge211::Mouse_button button, ge211::Posn<int> position){
    ...
    if (model.get_set().find_height() < 100) {
        model.set_game(true);
    }
    ...
    if (!model.check_is_game_over()){
        ...
        /// the button is not being clicked, generate a random new fruit
            ...
            if (!model.check_is_paused() && model.get_set().find_max_tier() < model
            .config
            .fruit_tier){
                ...
                new_fruit.get_radius() + 5){
                    ...
                    model.get_set_ptr()->add_fruits(model.get_generated_fruit());
...
UI::on_mouse_move(ge211::Posn<int> position){
    ...
    if (!model.check_is_paused()){
        if (position.x > model.get_generated_fruit().get_radius() && position.x <
            model.config.scene_dims.width - model.get_generated_fruit().get_radius()){
